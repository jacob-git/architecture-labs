...
def _run_pytest(path: Path, candidate_file: Path) -> CheckResult:
    env = os.environ.copy()
    env["ADP_B2_CANDIDATE_FILE"] = str(candidate_file)
    proc = subprocess.run(
...
def _third_party_dependency_check(candidate_file: Path) -> CheckResult:
    ...
    return CheckResult(
        "dependency_policy",
        not hits,
        "standard-library-only" if not hits else f"forbidden imports: {sorted(hits)}",
    )


def evaluate(candidate_file: Path, mode: str = "phase-b2") -> EvaluationResult:
    root = Path(__file__).resolve().parent
    candidate_file = candidate_file.resolve()
    return EvaluationResult(
        mode=mode,
        checks=(
            _run_pytest(root / "b2_tests_visible.py", candidate_file),
            _run_pytest(root / "b2_tests_hidden.py", candidate_file),
...
def main() -> None:
    root = Path(__file__).resolve().parent
    candidate = Path(sys.argv[1]).resolve() if len(sys.argv) > 1 else root / "b2_reference_solution.py"
    mode = sys.argv[2] if len(sys.argv) > 2 else "phase-b2"
